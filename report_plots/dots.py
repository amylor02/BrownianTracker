...
for line in lines:
    ...
    frame = int(parts[-1]) * 0.1
    displacements.append(displacement)
    frames.append(frame)
    particle_ids.append(particle_id)


plt.scatter(displacements, frames, color='b', marker='o')


for i, particle_id in enumerate(particle_ids):
    plt.text(displacements[i], frames[i], str(particle_id), fontsize=8, ha='right', va='bottom')

plt.title('Time Tracked (sec) = f(Displacement (m))')
plt.xlabel('Displacement in xy axis (m)')
plt.ylabel('Time Tracked (sec)')
plt.grid(True)
plt.show()